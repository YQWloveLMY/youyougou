...
export default {
    ...
    methods: {
        changeActive(saleAttrValue, arr) {
            //产品切换属性值高亮
            //用排他法，自己高亮，兄弟元素全都不亮，这里需要获取两个数据，第一个数据为点击的是哪一个对象
            //第二个为所有的对象，即第一个为自己，第二个为自己和兄弟
            arr.forEach((currentValue) => {
                // 先让所有的都为0，全都不高亮
                currentValue.isChecked = 0;
            });
            // 再让自己为1，即高亮
        ...
        //加入购物车
        async addShopcar() {
            //1.发请求---将产品加入到数据库（存储到服务器）
            //2.服务器存储成功---进行路由跳转,并且传递参数
            //3.失败，提示用户
            // 由于vux中有async和await，所以调用函数会返回一个promise
            // 同理这也需要async和await
            try {
                ...
                // 进行路由跳转,同时将产品信息带个下一路由组件
                this.$router.push({
                    name: "addcartsuccess",
                    // 直接通过query参数传递，由于是个对象，导致地址栏不好看
                    // query: { skuInfo: this.skuInfo, skuNum: this.skuNum },
                    //所以这里只传递id，其他的会话存储，不能存对象,要先把对象转字符串
